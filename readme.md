...
设备照常按"发邮件"的方式告警，**账号密码不用管**（默认什么都不校验，留空也行）；stmp2log 把信收下来、储存成可检索的日志，再按你配的规则推送到 ntfy / Bark / 钉钉 / 飞书 / 短信邮件等等。
stmp2log不会把收到的邮件原样发给收件人，除非你配置了"邮件"通知渠道，那发的是按你的模板渲染出来的一条新通知。
...
| `stmp_hostname` | 系统 hostname | SMTP 问候语里报的主机名，也是自签证书里的名字和转推来源名。**在界面上改只影响问候语和转推来源名** —— 证书里的名字是启动时签进去的，不跟着变 |
...
| `web_url` | 空 | 部分通知app里链接指向的地址，仅作为消息按钮展示 |
| `max_entries` | `5000` | 最多保留条目 |
| `max_days` | `0` | 最多保留天数，0 表示不按时间删 |
| `keep_raw` | `0` | 保留原始邮件源码 |
| `keep_attachments` | `0` | 保存附件内容 |
| `retry_queue` | `0` | 断网时最多攒多少条待重发，网络恢复后自动补发。`0`（默认）表示不限制，一条都不丢；填数字就是上限，超出丢最旧的 |
...
支持的渠道：ntfy、Bark、钉钉、飞书、企业微信、短信邮件等等。

**断网重发**。推不出去的通知和转推不会直接丢掉，而是先攒进一个队列，网络恢复后自动补发，最迟五分钟一定会再试一次。`retry_queue` 是队列上限，**默认 0 = 不限制**，一条都不丢；填一个数字就是上限，不限制时内存也就没有上限，队列里放的是整封原始邮件和渲染好的正文，内存小的设备上建议设一个数。只有网络错误的失败才进队列：5xx、429。webhook 地址写错、token 无效这类配置错误重发一百次也是错的，只会把队列占满，所以直接放弃并在界面上标成"已放弃"。"设置 → 断网重发"能看到还有几条在等、最旧的那条是什么时候的，也能点"立即重试"不等退避。  
